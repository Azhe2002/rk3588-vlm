...
class Arm:
    ...
    @property
    def name(self) -> str:
        return f"{self.transform_id}::{self.condition_id}::{self.question_id}"
...
def analyze_runs(round_files: Iterable[Path], output_dir: Path) -> dict[str, Any]:
    ...
    transitions = pairwise_transitions(records)
...
def arm_for(record: Mapping[str, Any]) -> Arm:
    return Arm(
        transform_id=str(record.get("transform_id", "raw")),
...
def summarize(records: Iterable[Mapping[str, Any]]) -> list[dict[str, Any]]:
    ...
    for record in records:
        groups[arm_for(record)].append(record)
    ...
    for arm in sorted(groups, key=lambda item: item.name):
        ...
        correct_low, correct_high = wilson_interval(correct, len(known))
        latencies = [
        ...
        rows.append(
            {
                ...
                "semantic_known_n": len(known),
                "semantic_correct_n": correct,
                "semantic_correct_rate": correct / len(known) if known else None,
...
def summarize_sessions(records: Iterable[Mapping[str, Any]]) -> list[dict[str, Any]]:
    ...
    for record in records:
        ...
        groups[(source_session, arm_for(record).name)].append(record)
    rows: list[dict[str, Any]] = []
...
def pairwise_transitions(records: Iterable[Mapping[str, Any]]) -> list[dict[str, Any]]:
    ...
    for record in records:
        ...
        key = (
            str(record.get("session_id")),
            str(record.get("run_id")),
            str(record.get("source_frame_id", record["frame_id"])),
    ...
    for arm_a, arm_b in combinations(sorted(by_arm, key=lambda item: item.name), 2):
        ...
        if not common:
            continue
        row: dict[str, Any] = {"arm_a": arm_a.name, "arm_b": arm_b.name, "paired_n": len(common)}
