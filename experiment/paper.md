...
> **v0.4 更新**: 追加三组机制闭合实验（无需板端改码）：σ∈{1,2,4,5} 精细扫描（8 组模糊总谱）——**任何模糊（σ=1）即让句子归零**；探测式措辞 D × 4 分辨率（320/480/640/800）——**D 在所有分辨率消除格式崩塌**，且**同会话内措辞翻转语义判定**（320 下 A→Yes 100% vs D→No 88%）；静态图无缓存对照（cache_prompt=false 实测绕过 KV cache）——**"实时帧才复现"获无缓存确认**（p=0.0004）。
...
本文进一步通过受控干预定位机制：**(a) 温度扫描**（实验7）：256M 的遵循率随采样温度单调变化（temp 0.0 时 85% vs temp 1.0 时 39%，p=0.0003），但 greedy 解码下仍有 15% 句子输出；**500M 的崩塌与温度完全无关**（greedy 下依然 100% 描述句）——崩塌的确定性成分随模型规模增大而增强。**(b) GStreamer 滤镜变体**（实验2/3 + σ 精细扫描，11 组）：在显示分辨率保持 640×480 不变的条件下，将采集/有效像素降至 320×240（缩放或裁剪）或经 160×120 重采样去纹理，遵循率全部恢复至 100%（vs 同会话基线 67%，p=0.0004）；**σ=1 的几乎不可见模糊即使句子输出归零**——描述先验对纹理清晰度极度敏感；σ≥7 时格式全合规但感知失败（全答 "No."，与 160×120 低分辨率同型）——说明"感知失败→单词输出"与"感知成功+信息过载→描述句"是两种不同状态。**(c) 措辞×分辨率**（实验5 补充）：探测式句式 "Does the image contain…?" 在 320/480/640/800 全分辨率消除格式崩塌，且在 320 下将语义从 100% "Yes." 翻转为 88% "No."（同日同帧 A 措辞对照）——措辞决定任务模板与判定门槛。**(d) 静态图无缓存复核**：cache_prompt=false（实测绕过 KV cache）下高细节 640 静态图 30/30 全合规 vs 实时链路 67%（p=0.0004）——"实时帧才复现"确认为真实现象，**输入链路的逐帧变化是崩塌的必要条件**。这些结果强烈支持**视觉细节驱动的描述先验激活（H2）**：消除细节（不改变显示分辨率）即可恢复格式遵循，且措辞模板与输入链路的实时性是独立干预面。
...
### 4.10 实验3 补充：σ 精细扫描（描述先验对纹理清晰度极度敏感）

在 4.9 的 σ∈{3,7,15} 基础上补 σ∈{1,2,4,5}，形成完整模糊谱（256M@640，会话内基线 67%）：

| σ | 严格遵循 | 单词 | 句子 | 语义 yes | vs 基线 Fisher |
|---|---------|------|------|----------|----------------|
...
（帧指标验证 σ 梯度单调：相邻像素差分均值 2.86→1.89→1.16→1.01，σ=0 约 3.2）

**关键结论**：
1. **任何模糊（σ=1，肉眼几乎不可见）即让句子输出从 11/33 归零**——描述先验对纹理清晰度**极度敏感**，几乎是一个阶跃而非渐变（σ=0→1 是最陡峭的跳变）
2. **σ∈[1,4] 是"格式恢复 + 语义保留"的宽操作窗**（严格 87.5–100%、语义 yes ≥87.5%）：轻模糊是实际部署中可行的补救手段
3. **σ=5 是感知过渡带**：格式仍全单词但语义开始翻 "No."（10/31 yes）；σ≥7 感知失败全 No.——与 exp1 160×120、σ=7/15 同型，再次确认"感知失败→单词"状态

### 4.11 实验5 补充：探测式措辞 D × 4 分辨率（恢复普适 + 措辞决定语义门槛）

256M 上以 D 措辞（"Does the image contain…?"）跑 320/480/640/800，与同日 A 措辞基线对照：

| 分辨率 | D 严格 | D 语义 yes | A 严格（同日/历史） | A 语义 yes |
|--------|--------|-----------|---------------------|-----------|
| 320×240 | 12% (4/33)，全单词 | **12%** | **100%**（同日） | 100% |
| 480×360 | **100%** (33/33) | 100% | 100%（历史） | 100% |
| 640×480 | **100%** (33/33，同日) | 100% | 67%（同日） | 100% |
| 800×600 | **100%** (33/33) | 100% | 0%（历史） | 100% |

**关键结论**：
1. **D 措辞在所有分辨率下完全消除格式崩塌**（≥480 全部 100% 单词，320 全单词）——恢复具普适性，今日 640 组 100% 甚至强于 exp5 的 82%
2. **新发现：措辞决定语义判定门槛**——同会话同分辨率 320×240 下，A 措辞 33/33 "Yes."、D 措辞 29/33 "No."（同一图像内容！）。探测式句式（"Does X contain Y"）提高证据门槛：低细节时保守否定；断言式（"Is there…in the center"）低门槛直接肯定。480+ 细节充分 → D 也肯定
3. **机制解读**：D 模板把模型拉入"存在性判定"模式——任务模板层面压制"场景描述"先验；而 320 的语义翻转说明该模板同时改变了模型的判定策略（非仅格式层面）→ **措辞干预与输入细节干预（§4.9/4.10）是两个独立、可叠加的干预面**

### 4.12 对照复核：静态图"不复现"非缓存假象（cache_prompt=false）

§4.7 的静态图 10/10 合规结论曾被质疑为 prompt cache 命中（相同请求 0.1s 缓存返回）。本复核用 `cache_prompt=false` 强制每轮全量计算（实测耗时 0.16s → 5.9s/轮，绕过确认）：

| 静态图 | 轮次 | 输出 | 对照 |
|--------|------|------|------|
| rk3588_640x480.jpg（真 640×480 高细节采集） | 30 | **30/30 单词，全 "Yes."** | 实时链路 640：67% 单词（Fisher p=0.0004） |
| 今日 exp2_C 帧（320 采集→放大 640，低细节） | 30 | 30/30 单词，全 "No." | —（与 D@320 同型：低细节→保守 No.） |

**关键结论**：
1. **"静态图不复现、实时帧才复现"获得无缓存确认**（p=0.0004）：固定单一高细节 640 图像 → 描述先验不激活；实时链路每轮内容微差的帧 → 激活
2. **输入链路的逐帧变化是崩塌的必要条件之一**——与措辞（§4.11）、细节量（§4.9/4.10）、温度（§4.8）并列的第四个干预面
3. 低细节静态帧全 No. 与 D@320 同型（低细节→保守否定），与静态/实时无关——静态-实时差异必须以同细节水平比较（本表第一行满足）
...
7. **描述先验对纹理清晰度极度敏感**（σ 精细扫描）：σ=1 的几乎不可见模糊即让句子输出归零（§4.10）
8. **采样温度是放大因素而非主因**（实验7）：256M 遵循率随温度单调（0.0→85%, 1.0→39%），但 greedy 仍残留句子；500M 崩塌与温度无关（§4.8）
9. **感知失败与格式崩塌互斥**：感知失败（低分辨率/强模糊）→ 格式必然合规（诚实 "No."）；格式崩塌只出现在"感知成功 + 信息过载"时
10. **措辞决定任务模板与语义门槛**：探测式句式（D）在所有分辨率消除格式崩塌，且在 320 下将语义从 100% Yes 翻转为 88% No.（§4.11）——措辞同时作用于格式层与判定层
11. **输入链路的逐帧变化是崩塌必要条件之一**：静态图（无缓存复核）高细节 640 图像 30/30 合规 vs 实时链路 67%（§4.12）
...
| — | 措辞框架（探测式 vs 位置断言式） | "Does the image contain...?" 恢复 | **显著恢复且普适**（实验5 D 组 82% p<0.01；实验7b 补充：D × 320/480/640/800 全分辨率消除崩塌，§4.11）；附带发现：D 措辞在 320 翻转语义为 No.（措辞决定判定门槛） |
| — | 温度扫描 | 低温度 → 遵循恢复 | **完成**（实验7：256M 单调恢复至 85%，greedy 仍残留；500M 无效；§4.8） |
| — | 静态图 vs 实时帧 | 静态不复现 → 逐帧变化必要 | **确认**（cache_prompt=false 无缓存复核：静态 30/30 合规 vs 实时 67%，p=0.0004；§4.12） |
...
5. **细节驱动机制获直接干预证据（实验2/3 + σ 精细扫描）**：显示分辨率固定 640×480 时，有效像素降至 320 级（缩放/裁剪/去纹理）→ 遵循恢复 100%；σ=1 的几乎不可见模糊即让句子输出归零——**消除细节（分辨率不变）即恢复遵循，且对纹理清晰度极度敏感**，H2 从"待验证"升格为强支持。同时揭示"感知失败→单词"与"感知成功+信息过载→描述句"是两种互斥状态
6. **采样温度是放大器而非主因（实验7）**：256M 遵循率随温度单调（85%→39%），greedy 仍残留 15% 句子；500M 崩塌与温度无关——崩塌的确定性成分随模型规模增强
7. **探测式措辞是普适且独立的干预面**：D 措辞在 320/480/640/800 全分辨率消除格式崩塌（§4.11），且在 320 将语义从 100% Yes 翻转为 88% No.——措辞同时作用于格式层与判定层（任务模板改变）
8. **"逐帧变化"是崩塌的必要条件**：无缓存复核确认静态高细节图像 30/30 合规 vs 实时 67%（p=0.0004）——输入链路的实时性本身参与崩塌机制
9. **跨会话基线漂移需正视**：同配置 256M@640 t=0.1 严格率三会话 36%↔45%↔67%——恢复效应结论均基于会话内对照
10. **实验方法警示**：静态图测试不复现该现象（无缓存复核后依然），板端实时链路实验不可替代

对边缘部署的实际启示：若应用要求严格格式输出，低分辨率（320×240）当前是更可靠的选择；但需注意 500M@320 存在语义摇摆风险（41%），并评估感知能力余量（256M@160/320 感知稳定、500M@320 感知不稳）。高分辨率部署必须配套解码约束或语义解析兜底；若必须用 640 级采集，可在输入端降采样/轻度模糊后送入模型（σ∈[1,4] 宽窗显示该操作可恢复遵循且语义无损）；也可改用探测式问题模板（D 措辞全分辨率消除崩塌，但注意其 320 下的保守化语义——需按应用场景验证语义方向）。
...
- **图像处理变体**: 已完成 σ∈{1..15} 8 档 + 缩放/裁剪/重采样（§4.9/4.10）；未测试彩色/亮度扰动、JPEG 质量梯度等更广图像扰动面；σ=5 感知过渡带只测了 1 档，未精细定位"语义翻 No."起点
- **措辞×分辨率交互**: D 措辞已测 4 分辨率（§4.11）；A 措辞的 320 同日对照仅 1 组；D 措辞的语义门槛（320 翻转 No.）只验证了 1 个分辨率点，门槛在 256/288 等更细梯度的位置未知
- **跨会话漂移**: 同配置 256M@640 t=0.1 三会话 36%/45%/67%——绝对数值跨会话不稳定，未量化环境因素（光照/场景微差）贡献
- **静态-实时机制**: 已确认逐帧变化是必要条件（§4.12），但"多少帧间变化量足够触发"未测（JPEG 质量扰动梯度可量化）
...
| 滤镜变体日志 ×11（实验2/3，含 σ 精细） | `experiment/data/exp2_*.log`、`exp3_*.log` + 帧采样 `.tgz` |
| D 措辞×分辨率 ×3 + 同日对照 ×2 | `experiment/data/expD_256_*.log`、`expA_256_320_sameS.log`、`expD_256_640_sameS.log` |
| 静态无缓存对照 ×2 | `experiment/data/exp_static_nocache.log`（会话帧/旧图各 30 轮） |
...
| 驱动脚本 | `experiment/run_exp1.py`、`experiment/run_exp5.py`、`experiment/run_exp7.py`、`experiment/run_exp2_3.py`（支持 `--only`）、`experiment/run_expD_res.py`（D 措辞×分辨率）、`experiment/run_static_nocache.py`（静态无缓存）、`experiment/board_exp_run.sh`（支持第 9/10 参数 temp/gst_extra） |
